...
with filter_cols[2]:
    tier_pick = st.selectbox("Tier", ["STEWARD_REVIEW", "AUTO_MERGE", "SEPARATE"])
with filter_cols[3]:
    row_limit = st.selectbox("Show", [10, 25, 50], index=0)

# --- Tabs for actions ---
tab_review, tab_merged, tab_separated = st.tabs(["Pending Review", "Merged", "Separated"])

with tab_review:
    try:
        candidates = get_candidates(tier_pick, name_search, state_pick, row_limit)
        ...
        if not candidates:
            ...
            for c in candidates:
                with st.expander(
                    ...
                    b1.button("Merge", key=f"m_{c['pair_id']}", type="primary")
                    b2.button("Keep Separate", key=f"s_{c['pair_id']}")
                    b3.button("Escalate", key=f"e_{c['pair_id']}")

    except Exception as e:
        st.error(f"Error: {e}")

with tab_merged:
    st.info("Merged records will appear here after steward actions are wired up (Week 4).")

with tab_separated:
    st.info("Separated records will appear here after steward actions are wired up (Week 4).")
